...
export function AccountMenu() {
  ...
  async function handleRegister(event: FormEvent<HTMLFormElement>) {
    ...
    if (!response.ok) return setMessage(data.error ?? 'Erreur création compte.');
    setUser(data.user);
  ...
  async function handleLogin(event: FormEvent<HTMLFormElement>) {
    ...
    const data = await response.json();
    if (!response.ok) return setMessage(data.error ?? 'Erreur connexion.');
    setUser(data.user);
      ...
      {open ? (
        <div className='absolute right-0 z-20 mt-2 w-80 rounded-lg border bg-white p-4 shadow-lg'>
          ...
          <form className='mb-4 space-y-2' onSubmit={handleLogin}>
            ...
            <button className='rounded bg-slate-900 px-3 py-2 text-xs text-white'>Se connecter</button>
          </form>
          <form className='space-y-2' onSubmit={handleRegister}>
            ...
            <button className='rounded bg-blue-700 px-3 py-2 text-xs text-white'>Créer un compte</button>
          </form>
